...
To begin using proxmox module with default parameters, declare the hypervisor's class with `include proxmox5::hypervisor`.
...
node "pve_node" {
  ...
  proxmox5::hypervisor::group { 'sysadmin': role => "Administrator", users => [ 'marvin@pam', 'arthur@pam' ] }
...
* `proxmox5::hypervisor`: Install the Proxmox hypervisor on the system.

### Defined types

* `proxmox5::hypervisor::group`: Manage groups for Proxmox WebGUI and set permissions.
...
* `proxmox5::hypervisor::user`: Manage user for Proxmox WebGUI.

```
proxmox5::hypervisor::user { 'marvin':
  group => 'sysadmin',
}
```

  Mainly used by the `proxmox5::hypervisor::group` defined type to create the group, permissions and also create/add the users to a group. Because to add a user to a group via this defined type, the group should already exist.
...
* `pveproxy_default_content`: Template file use to generate the previous configuration file. Default to 'proxmox5/hypervisor/pveproxy_default.erb'.
...
* `pve_modules_file_content`: Template file used to generate the previous configuration file. Defaults to 'proxmox5/hypervisor/proxmox_modules.conf.erb'.
...
* `init_lvm_script_content` : Temple file use to generate the previous configuration file. Default to 'proxmox5/hypervisor/initramfs-lvm-manual.erb'.
...
Other notes
-----------
By default `proxmox5::hypervisor` comes with several modules kernel load at boot time. Mainly iptables's modules to allow it in the CT.
