...
class proxmox5::hypervisor::install {
  ...
  if $::is_pve_kernel == 'true' {
    ...
    package { $proxmox5::hypervisor::ve_pkg_name:
      ensure => $proxmox5::hypervisor::ve_pkg_ensure,
    } ->

    # Remove useless packages (such as the standard kernel, acpid, ...)
    package { $proxmox5::hypervisor::old_pkg_name:
      ensure => $proxmox5::hypervisor::old_pkg_ensure,
      notify => Exec['update_grub'],
    }

    # Ensure that some recommended packages are present on the system
    ensure_packages( $proxmox5::hypervisor::rec_pkg_name )
    ...
    notify { 'Please REBOOT':
      message  => "Need to REBOOT the system on the new PVE kernel (${proxmox5::hypervisor::kernel_pkg_name}) ...",
      loglevel => warning,
    } ->

    package { $proxmox5::hypervisor::kernel_pkg_name:
      ensure => $proxmox5::hypervisor::ve_pkg_ensure,
      notify => Exec['update_grub'],
    }
...
} # Private class: proxmox5::hypervisor::install
