...
class proxmox5::hypervisor::config {
  ...
  if $::is_proxmox == 'true' {
    ...
    file { $proxmox5::hypervisor::pveproxy_default_path:
      ensure  => present,
      content => template($proxmox5::hypervisor::pveproxy_default_content),
      notify  => Service[$proxmox5::hypervisor::pveproxy_service_name],
    }
    ->
  ...
  if $proxmox5::hypervisor::labs_firewall_rule == true {
...
} # Private class: proxmox5::hypervisor::config
