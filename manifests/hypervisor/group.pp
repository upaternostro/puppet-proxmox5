# == Define: proxmox5::hypervisor::group
#
...
#
define proxmox5::hypervisor::group ( $group = $title, $acl_path = '/', $permission_file = '/etc/pve/user.cfg', $users = '', $role ) {
  ...
  if $::is_proxmox == 'true' {
    ...
    proxmox5::hypervisor::user { $users:
      group => $group,
    }
...
} # Public ressource: proxmox5::hypervisor::group
